...
pub fn get_topic_by_slug(conn: &Connection, slug: &str) -> rusqlite::Result<Option<Topic>> {
    let mut stmt =
        conn.prepare("SELECT id, name, slug, \"type\", created_at FROM topics WHERE slug=?1")?;
    let mut rows = stmt.query_map(params![slug], |row| {
        Ok(Topic {
            id: row.get(0)?,
            name: row.get(1)?,
            slug: row.get(2)?,
            topic_type: row.get(3)?,
            created_at: row.get(4)?,
        })
    })?;
...
pub fn get_all_topics(conn: &Connection) -> rusqlite::Result<Vec<Topic>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, slug, \"type\", created_at FROM topics ORDER BY created_at DESC",
    )?;
    let rows = stmt.query_map([], |row| {
        Ok(Topic {
            id: row.get(0)?,
...
pub fn update_concept_domain(
    conn: &Connection,
    concept_slug: &str,
    domain_id: &str,
) -> rusqlite::Result<()> {
    conn.execute(
        "UPDATE concepts SET domain_id=?1 WHERE slug=?2",
        params![domain_id, concept_slug],
    )?;
    Ok(())
}
...
pub fn get_concept_tree(conn: &Connection, topic_slug: &str) -> rusqlite::Result<TreeNode> {
    let topic =
        get_topic_by_slug(conn, topic_slug)?.ok_or_else(|| rusqlite::Error::QueryReturnedNoRows)?;
    ...
    for domain in domains {
        ...
        let concepts: Vec<TreeNode> = stmt
...
pub fn award_achievement(conn: &Connection, id: &str) -> rusqlite::Result<()> {
    conn.execute(
        "UPDATE achievements SET earned_at=datetime('now') WHERE id=?1",
        params![id],
    )?;
    Ok(())
}
